...
//		
	...
	// @CreateBatchMandatoryField
		@Given("Admin creates POST Request with missing additional fields {string}")
		public void admin_creates_post_request_with_missing_additional_fields(String jsonKey) throws FileNotFoundException {
			request = given().spec(CommonSpec()).body(JsonReader.getRequestBody(jsonFileName, jsonKey))
					                            .header("Authorization", "Bearer " + PC_token.getToken());
		}
...
//@GetBatchByBatchId
	...
	public void admin_sends_https_get_request_with_endpoint() {
		...
		response = request.when().log().all().get(API_Path.apiPath.getBatchByBatchID+"{batchId}",batch1.getBatchId())
				.then().log()
    ...
    // @UpdateBatchValid
	@Given("Admin creates PUT Request with auth, valid BatchId and {string}")
	public void admin_creates_put_request_with_auth_valid_batch_id_and(String jsonKey) throws FileNotFoundException {
		request = given().spec(CommonSpec()).body(JsonReader.getRequestBody(jsonFileName, jsonKey))
                .header("Authorization", "Bearer " + PC_token.getToken());
	}
